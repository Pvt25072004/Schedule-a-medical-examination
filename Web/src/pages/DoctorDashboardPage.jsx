...
const DoctorDashboardPage = ({ navigate }) => {
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 snap-x hide-scrollbar">
        {dates.map((date) => {
          ...
          if (date !== "all") {
            ...
            label = `${days[d.getDay()]}, ${String(d.getDate()).padStart(2, "0")}/${String(d.getMonth() + 1).padStart(2, "0")}`;
          }
          return (
            <button
              key={date}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {[
          ...
          return (
            <Card
              key={card.title}
              className="flex items-center gap-5 hover:shadow-xl hover:scale-[1.02] transition-all duration-300"
            >
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${card.color}`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="flex flex-col h-full">
          ...
          <div className="space-y-4 flex-1">
            {loadingAppointments ? (
              <p className="text-sm text-slate-500 text-center py-8 animate-pulse">
                Đang tải lịch hẹn...
              </p>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {apt.relationship && apt.relationship !== "Bản thân" ? (
                        ...
                        <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-md border border-emerald-200">
                          Chính chủ
                        </span>
                      )}
                    </div>
                    <p className="text-sm font-medium text-[#48a1f3] mt-1">
                      {formatDateVN(apt.appointment_date)} ·{" "}
                      {apt.appointment_time?.slice(0, 5)}
                    </p>
                    <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                      <MapPin className="w-3 h-3" /> {apt.hospital?.name}
                    </p>
                  </div>
                  <div>
                    ...
                    {apt.status === "pending" && (
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold">
                        Chờ duyệt
                      </span>
                    )}
                  </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6 p-2">
          <div>
            ...
            <p className="text-sm text-blue-100/80 max-w-xl font-medium">
              Bạn có thể gửi yêu cầu cập nhật hồ sơ bác sĩ, xin nghỉ việc hoặc
              liên kết thêm bệnh viện mới trực tiếp trên hệ thống. Đội ngũ admin
      ...
      <Card>
        ...
        <div className="space-y-4">
          ...
          {filteredSchedules.map((entry) => (
            <div
              key={entry.id}
              className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-4"
            >
      ...
      <Card>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Gửi yêu cầu làm việc tại các cơ sở y tế
            </p>
          </div>
          <Button
            size="sm"
              ...
              >
                <div>
                  <p className="font-bold text-slate-800 flex items-center gap-2">
                    {app.hospital?.name}
                    {app.type === "leave" ? (
                      <span className="text-[10px] uppercase font-black text-red-600 bg-red-100 px-2 py-0.5 rounded-md">
                        Xin nghỉ
                      </span>
                    ) : (
                      <span className="text-[10px] uppercase font-black text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-md">
                        Ứng tuyển
                      </span>
                    )}
                  </p>
                ...
                <div>
                  ...
                  {app.status === "rejected" && (
                    <span className="text-xs font-bold text-red-600 bg-red-50 px-3 py-1.5 rounded-full border border-red-100">
                      Từ chối
                    </span>
                  )}
                </div>
      ...
      <Card>
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Thông tin hiển thị công khai trên cổng đặt khám
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-10">
            ...
            <div className="flex-1 grid gap-6 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Vd: Trưởng khoa Ngoại..."
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  disabled
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-100 text-slate-500 cursor-not-allowed font-medium"
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  rows={4}
                  placeholder="Chia sẻ kinh nghiệm làm việc để bệnh nhân tin tưởng hơn..."
      ...
      <Card>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Tất cả nhận xét sau khi khám xong
            </p>
          </div>
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-slate-900">
                    {rev.user?.full_name || "Bệnh nhân ẩn danh"}
                  </h3>
                  <span className="text-xs text-slate-400 font-medium">
                    {formatDateVN(rev.created_at)}
                  </span>
                </div>
                <div className="flex gap-1 mb-2">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i < rev.rating ? "fill-amber-400 text-amber-400" : "fill-slate-200 text-slate-200"}`}
                    />
                  ))}
                </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium">
                Lịch sử thu phí từ các cuộc hẹn (Chỉ hiển thị giao dịch thành
                công)
              </p>
            </div>
            <Button
  ...
  return (
    <div className="min-h-screen bg-[#f4f8fb] font-sans flex flex-col md:flex-row">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-72 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]`}
      >
        <div className="p-6 md:p-8 flex items-center gap-4 hidden md:flex border-b border-slate-100">
          ...
          <div>
            <h1 className="font-black text-[#143250] text-xl tracking-tight">
              STL Clinic
            </h1>
            <p className="text-xs font-bold text-[#48a1f3] uppercase tracking-wider">
              Doctor Portal
            </p>
          </div>
        </div>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 bg-[#143250]/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-8 animate-in zoom-in-95">
            <h3 className="text-xl font-black text-[#143250] mb-2">
              Ứng tuyển Bệnh viện
            </h3>
            <p className="text-sm font-medium text-slate-500 mb-6">
              Liên kết để có thể tạo lịch khám tại đây
            </p>
