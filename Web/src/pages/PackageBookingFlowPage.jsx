...
const PackageBookingFlowPage = () => {
  ...
  const calculateEndTime = (startTimeStr, durationMins) => {
    ...
    const lunchDuration = 90; // Sửa thành 90 mins giống backend
    ...
    try {
      ...
      if (!finalDoctorId) {
        if (availableDoctors.length > 0) {
          ...
          showError("Hiện không có bác sĩ nào rảnh vào thời gian này. Vui lòng chọn giờ khác.");
              ...
              {crossShift && (
                <div className="mt-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-lg flex items-start gap-3">
                  ...
                  <p className="text-yellow-800 text-sm">
                    <strong>⚠️ Lưu ý:</strong> Vì gói khám kéo dài {servicePackage?.duration_minutes} phút, ca khám của bạn sẽ được tiếp tục vào đầu ca chiều (từ 13:00) sau giờ nghỉ trưa của bệnh viện.
                  </p>
                </div>
              )}
                ...
                <div className="pb-4 border-b border-blue-200 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 mt-2 flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 shrink-0" /> Lịch hẹn của bạn đã được xác nhận.
                    </p>
                  </div>
                </div>
